fetch('https://backendweatherapp-p3.vercel.app/weather')
...
function updateDeleteCityEventListener() {
	for (let i = 0; i < document.querySelectorAll('.deleteCity').length; i++) {
		document.querySelectorAll('.deleteCity')[i].addEventListener('click', function () {
			fetch(`https://backendweatherapp-p3.vercel.app/weather/${this.id}`, { method: 'DELETE' })
				.then(response => response.json())
				.then(data => {
					if (data.result) {
...
document.querySelector('#addCity').addEventListener('click', function () {
	...
	fetch('https://https://backendweatherapp-p3.vercel.app/weather', {
		method: 'POST',
