  ...
  return (
    <form onSubmit={handleSubmit} className="w-2/5 flex justify-center align-center">
      ...
      <div className={`w-1/5 rounded-lg ml-2 ${theme === 'dark' ? 'bg-neutral-500' : 'bg-neutral-200'}`}>
        <button
          type="submit"
          className={`w-full -translate-y-[6px] active:-translate-y-[2px] font-bold rounded-lg p-3 border-solid border-l ${theme === 'dark' ? 'border-neutral-800 text-neutral-200 bg-neutral-800 border-neutral-200' : 'border-neutral-100 text-neutral-800 bg-neutral-200 border-neutral-800'}`}
        >
          SEARCH
        </button>
      </div>
